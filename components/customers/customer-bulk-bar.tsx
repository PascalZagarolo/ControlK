...
import { bulkUpdateCustomers } from '@/lib/actions/customers';
import type { CustomerStatus, CustomerTag, TodoUser } from '@/lib/types';
...
export function CustomerBulkBar({
  ...
  const apply = (update: Parameters<typeof bulkUpdateCustomers>[0]) => {
    start(async () => {
      await bulkUpdateCustomers(update);
      router.refresh();
      onClear();
      setOpen(null);
    });
  ...
  return (
    <div className="sticky bottom-6 z-30 mx-auto flex w-fit items-center gap-2 rounded-full border border-white/[0.10] bg-[rgba(20,21,23,0.96)] px-3 py-2 shadow-xl backdrop-blur-md">
      ...
      <BulkActionMenu
        label="Status"
        open={open === 'status'}
        toggle={() => setOpen(open === 'status' ? null : 'status')}
      >
        {(['aktiv', 'lead', 'inaktiv'] as CustomerStatus[]).map((st) => (
          <button
            key={st}
            type="button"
            onClick={() => apply({ customerIds: selectedIds, status: st })}
      ...
      <BulkActionMenu
        label="Owner"
        open={open === 'owner'}
        toggle={() => setOpen(open === 'owner' ? null : 'owner')}
      >
        <button
          type="button"
          onClick={() => apply({ customerIds: selectedIds, ownerId: null })}
        ...
        {members.map((m) => (
          <button
            key={m.id}
            type="button"
            onClick={() => apply({ customerIds: selectedIds, ownerId: m.id })}
            className="flex w-full items-center gap-2 rounded-[6px] px-2 py-1.5 text-left text-[12.5px] text-ink-100 hover:bg-white/[0.04]"
          >
      ...
      <BulkActionMenu
        label="Tag +"
        open={open === 'tag'}
        toggle={() => setOpen(open === 'tag' ? null : 'tag')}
      >
        {tags.length === 0 && (
        ...
        {tags.map((t) => (
          <button
            key={t.id}
            type="button"
            onClick={() => apply({ customerIds: selectedIds, addTagIds: [t.id] })}
      ...
      <button
        ...
        onClick={onClear}
        disabled={pending}
        className="rounded-full px-3 py-1 text-[11.5px] text-ink-300 transition-colors hover:text-ink-50"
      >
        Abbrechen
      </button>
    </div>
