...
export function TagPicker({
  ...
  const toggle = (tagId: string) => {
    ...
    start(async () => {
      await setCustomerTags({ customerId, tagIds: nextIds });
      router.refresh();
    });
  };
  ...
  const createAndAssign = () => {
    ...
    start(async () => {
      const res = await createCustomerTag({ name: newTagName.trim(), color: newTagColor });
      if (res.ok) {
    ...
    <div className="relative">
      <div className="flex flex-wrap items-center gap-1">
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="rounded-[4px] border border-dashed border-white/[0.18] px-2 py-[2px] font-mono text-[10px] uppercase tracking-[0.3px] text-ink-300 transition-colors hover:border-white/[0.32] hover:text-ink-50"
        >
          + Tag
        </button>
      </div>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {all.length === 0 && (
                <p className="px-2 py-2 text-[11px] text-ink-300">
                  Noch keine Tags. Erstell unten den ersten.
                </p>
              )}
            </div>
