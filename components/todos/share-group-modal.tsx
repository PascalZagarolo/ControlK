...
import { createShareLinkForGroup, revokeShareLink } from '@/lib/actions/share-links';
import type { ShareLink, TodoGroup } from '@/lib/types';
    ...
    >
      <form
        onSubmit={(e) => {
          ...
          start(async () => {
            const res = await createShareLinkForGroup({
              groupId: group.id,
              expiresInDays: days,
            });
            if (!res.ok) {
      ...
      >
          ...
          <div className="rounded-[10px] border border-[#5ee08a]/25 bg-[#5ee08a]/[0.05] p-3">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <button
                ...
                className="rounded-[6px] border border-white/[0.10] bg-white/[0.04] px-2.5 py-1.5 text-[11.5px] text-ink-100 transition-colors hover:bg-white/[0.08]"
              >
                {copied ? '✓ kopiert' : 'Kopieren'}
              </button>
            </div>
            <p className="mt-2 text-[11px] leading-[1.45] text-ink-300">
              Wer den Link hat, sieht die offenen Todos dieser Gruppe — read-only, ohne Login.
            </p>
          </div>
        ) : (
          <>
            <p className="text-[12.5px] leading-[1.5] text-ink-300">
              Erzeugt einen öffentlichen Read-Only-Link auf alle offenen Todos in dieser Gruppe.
              Ideal für Kunden oder externe Stakeholder, die Status sehen sollen — ohne Konto.
            </p>
        ...
        <ModalActions>
          ...
          {!createdToken && (
            <ModalPrimary pending={pending}>Link erstellen</ModalPrimary>
          )}
        </ModalActions>
      </form>
    </Modal>
